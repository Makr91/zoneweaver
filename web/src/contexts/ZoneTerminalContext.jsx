...
export const ZoneTerminalProvider = ({ children }) => {
  ...
  const createOrReuseTerminalSession = useCallback(
    async (server, zoneName) => {
      ...
      if (creatingSessionsSet.current.has(zoneKey)) {
        console.log(
          `⏳ ZLOGIN SESSION: Creation already in progress for ${zoneKey}`
        );
        return null;
      }
      ...
      if (sessionsMap.current.has(zoneKey)) {
        console.log(
          `✅ ZLOGIN SESSION: Reusing existing session for ${zoneKey}`
        );
      ...
      try {
        ...
        if (!sessionData.websocket_url) {
          console.error(
            `🚫 ZLOGIN SESSION: Missing websocket_url for ${zoneKey}!`
          );
          return null;
        }
        ...
        ws.onopen = () =>
          console.log(`🔗 ZONE TERMINAL: WebSocket connected for ${zoneKey}`);
        ws.onclose = () => {
        ...
        ws.onerror = (error) =>
          console.error(
            `🚨 ZONE TERMINAL: WebSocket error for ${zoneKey}:`,
            error
          );
        ...
        console.log(
          `🎉 ZONE TERMINAL: Session and WebSocket created successfully for ${zoneKey}`
        );
        return sessionData;
      } catch (error) {
        console.error(
          `💥 ZONE TERMINAL: Failed to create session for ${zoneKey}:`,
          error
        );
        return null;
      } finally {
  ...
  const preserveZoneHistory = useCallback((zoneKey) => {
    ...
    if (serializeAddon) {
      try {
        ...
        console.warn(
          `🔧 ZONE HISTORY: Failed to preserve for ${zoneKey}:`,
          error
        );
      }
    }
  ...
  const restoreZoneHistory = useCallback((zoneKey, terminal) => {
    ...
    if (history && terminal) {
      try {
        ...
        console.warn(
          `🔧 ZONE HISTORY: Failed to restore for ${zoneKey}:`,
          error
        );
      }
    }
  ...
  const forceZoneSessionCleanup = useCallback(
    async (server, zoneName) => {
      ...
      console.log(
        `🧹 ZLOGIN CLEANUP: Force cleaning up session state for ${zoneKey}`
      );
      ...
      console.log(
        `✅ ZLOGIN CLEANUP: Complete cleanup finished for ${zoneKey}`
      );
    },
  ...
  const initializeSessionFromExisting = useCallback(
    (server, zoneName, sessionData) => {
      ...
      if (!zoneKey || !sessionData || !sessionData.id) {
        console.error(
          `❌ ZLOGIN RECONNECT: Invalid data provided for ${zoneKey}`,
          {
            hasZoneKey: !!zoneKey,
      ...
      if (!websocketUrl) {
        console.error(
          `❌ ZLOGIN RECONNECT: Could not determine websocket URL for ${zoneKey}`
        );
        return;
      }
      ...
      if (websocketsMap.current.has(zoneKey)) {
        console.log(
          `✅ ZLOGIN RECONNECT: WebSocket already exists for ${zoneKey}`
        );
        return;
      }

      console.log(
        `🔄 ZLOGIN RECONNECT: Initializing session from existing data for ${zoneKey}`,
        {
      ...
      ws.onopen = () =>
        console.log(`🔗 ZLOGIN RECONNECT: WebSocket connected for ${zoneKey}`);
      ws.onclose = () => {
      ...
      ws.onerror = (error) =>
        console.error(
          `🚨 ZLOGIN RECONNECT: WebSocket error for ${zoneKey}:`,
          error
        );
      ...
      console.log(
        `🎉 ZLOGIN RECONNECT: Session initialized successfully for ${zoneKey}`
      );
    },
    [getZoneKey]
  );
  ...
  const pasteTextToZone = useCallback(
    (server, zoneName, text) => {
      ...
      if (ws && ws.readyState === WebSocket.OPEN) {
        console.log(
          `📋 ZLOGIN PASTE: Sending ${text.length} characters to ${zoneKey}`
        );
      ...
      console.warn(
        `⚠️ ZLOGIN PASTE: Cannot paste - WebSocket not ready for ${zoneKey}`
      );
      return false;
    },
    [getZoneKey]
