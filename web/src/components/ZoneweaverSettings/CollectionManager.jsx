...
const CollectionManager = ({ field, setMsg, setRequiresRestart, loading }) => {
  ...
  const { icon, itemSchema, itemLabelField, secretFields, keyLabel, itemNoun } =
    field.collection;

  const itemFields = useMemo(
    () => buildCollectionItemFields(itemSchema),
    [itemSchema]
  );
  ...
  const rowLabel = (item) =>
    (itemLabelField && item[itemLabelField]) || item._key;
