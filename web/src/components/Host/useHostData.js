...
export const useHostData = (currentServer) => {
  ...
  const processCPUHistoricalData = useCallback((cpuData) => {
    ...
    console.log(
      `📊 CPU: Built ${sortedData.length} historical points for CPU data`
    );
  ...
  const processMemoryHistoricalData = useCallback((memoryData) => {
    ...
    console.log(
      `📊 MEMORY: Built ${sortedData.length} historical points for memory data`
    );
  ...
  const loadHistoricalChartData = useCallback(async () => {
    ...
    try {
      ...
      if (
        ...
        const deduplicatedNetworkUsage = validNetworkUsage.reduce(
          (acc, usage) => {
            const existing = acc.find(
              (existing) => existing.link === usage.link
            );
            if (!existing) {
              acc.push({ ...usage });
            } else if (
              new Date(usage.scan_timestamp) > new Date(existing.scan_timestamp)
            ) {
        ...
        const networkUsageWithBandwidth = deduplicatedNetworkUsage.map(
          (usage) => ({
            ...usage,
            totalMbps:
              (parseFloat(usage.rx_mbps) || 0) +
              (parseFloat(usage.tx_mbps) || 0),
          })
        );
      ...
      if (poolIOResult.status === "fulfilled" && poolIOResult.value?.success) {
        ...
        const processedStorageIOData =
          processStorageIOHistoricalData(poolIOData);
        ...
        const deduplicatedPoolIO = poolIOData.reduce((acc, poolIO) => {
          const existing = acc.find(
            (existing) => existing.pool === poolIO.pool
          );
          if (!existing) {
            acc.push({ ...poolIO });
          } else if (
            new Date(poolIO.scan_timestamp) > new Date(existing.scan_timestamp)
          ) {
            const index = acc.indexOf(existing);
            acc[index] = { ...poolIO };
          }
      ...
      if (arcResult.status === "fulfilled" && arcResult.value?.success) {
        ...
        const deduplicatedARC = arcData.reduce((acc, arc) => {
          const existing = acc.find(
            (existing) => existing.scan_timestamp === arc.scan_timestamp
          );
          if (!existing) {
            acc.push({ ...arc });
          }
      ...
      console.log(
        "📊 HISTORICAL CHARTS: Completed loading historical data for all chart types"
      );
  ...
  const loadRecentChartData = useCallback(async () => {
    ...
    try {
      ...
      if (!hasTimestamps) {
        console.log(
          "🔄 RECENT CHARTS: No timestamps found, falling back to historical load"
        );
      ...
      console.log(
        "🔄 RECENT CHARTS: Completed loading recent data for all chart types"
      );
    } catch (error) {
      console.error(
        "🔄 RECENT CHARTS: Error loading recent chart data:",
        error
      );
    }
  }, [
    ...
    async (server) => {
      ...
      try {
        ...
        const sinceTime = new Date(
          now.getTime() - (timeWindowMs[timeWindow] || timeWindowMs["1hour"])
        );
  ...
  useEffect(() => {
    ...
    if (currentServer && makeZoneweaverAPIRequest) {
      ...
      loadHostData(currentServer).then(() => {
        ...
        console.log(
          "🔍 HOST: Initial data loading completed - preventing duplicate calls"
        );
      });
    }
  }, [currentServer]);
  ...
  const refreshAllData = useCallback(
    async (server = currentServer) => {
      ...
      console.log(
        "🔄 REFRESH: Starting combined refresh for host data and charts"
      );
  ...
  useEffect(() => {
    if (
      ...
      if (!initialDataLoaded) {
        console.log(
          "🔄 HOST: Auto-refresh waiting for initial data load to complete"
        );
      }
      return;
    }
    ...
    console.log(
      "🔄 HOST: Setting up auto-refresh every",
      refreshInterval,
      "seconds"
    );
