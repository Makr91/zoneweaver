...
const AggregateCreateModal = ({ server, existingAggregates, cdpServiceRunning, onClose, onSuccess, onError }) => {
  ...
  const generateNextAggregateName = () => {
    if (!existingAggregates) return 'aggr0';
  ...
  const loadAvailableLinks = async () => {
    ...
    try {
      ...
      if (result.success && result.data?.interfaces) {
        ...
        const physicalLinks = result.data.interfaces.filter(link => 
          link.class === 'phys' || (link.link && link.class !== 'vnic')
        );
        
        // Deduplicate by link name
        const uniqueLinks = physicalLinks.filter((link, index, self) => 
  ...
  const validateForm = () => {
    ...
    if (!nameRegex.test(formData.name)) {
      onError('Aggregate name must start with a letter and contain only letters, numbers, and underscores');
      return false;
    }
                  ...
                  <div className='control'>
                    <div className='select is-fullwidth'>
                      <select
                        ...
                        onChange={(e) => handleInputChange('lacp_mode', e.target.value)}
                        disabled={creating}
                      >
                ...
                <div className='control'>
                  <div className='select is-fullwidth'>
                    <select
                      ...
                      onChange={(e) => handleInputChange('lacp_timer', e.target.value)}
                      disabled={creating}
                    >
            ...
            <div className='field'>
              ...
              <div className='control'>
                <input
                  ...
                  onChange={(e) => handleInputChange('unicast_address', e.target.value)}
                  disabled={creating}
                />
              </div>
            ...
            {cdpServiceRunning && (
              <div className='notification is-warning mb-4'>
                <div className='content'>
                  <p><strong>CDP Service Conflict</strong></p>
                  <p>The Cisco Discovery Protocol (CDP) service is currently running and must be disabled before creating link aggregates.</p>
                    ...
                    <div className='control'>
                      <label className='checkbox'>
                        <input
                          ...
                          onChange={(e) => handleInputChange('disableCdp', e.target.checked)}
                          disabled={creating}
                        />
                        <span className='ml-2'><strong>Disable CDP service before creating aggregate</strong></span>
                      </label>
                    </div>
                    <p className='help'>This will stop the CDP service to allow aggregate creation. You can re-enable it later from the Services page.</p>
                  </div>
                </div>
              </div>
            )}
