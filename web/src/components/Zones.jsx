...
const Zones = () => {
  ...
  useEffect(() => {
    ...
    if (zoneParam && zones.length > 0) {
      ...
      if (zoneExists) {
        console.log(
          `🔗 URL PARAM: Auto-selecting zone from URL parameter: ${zoneParam}`
        );
        ...
        console.warn(
          `⚠️ URL PARAM: Zone '${zoneParam}' not found in current zones list`
        );
  ...
  useEffect(() => {
    ...
    if (hasVnc && hasZlogin) {
      ...
      if (
        !activeConsoleType ||
        (activeConsoleType !== "vnc" && activeConsoleType !== "zlogin")
      ) {
        console.log(
          "🔧 CONSOLE SWITCH: Both sessions available, defaulting to VNC"
        );
      ...
      console.log(
        "🔧 CONSOLE SWITCH: Only zlogin available, switching to zlogin"
      );
      ...
      console.log(
        "🔧 CONSOLE SWITCH: No sessions available, defaulting to VNC"
      );
      setActiveConsoleType("vnc");
    }
  ...
  useEffect(() => {
    ...
    if (
      ...
      console.log(
        "🔄 MODAL CLOSE: zlogin modal just closed, reconnecting preview terminal"
      );
