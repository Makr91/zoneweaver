        ...
        <div className='box mb-4'>
            <div className='level is-mobile mb-3'>
                ...
                <div className='level-right'>
                    ...
                    >
                        <span className='icon'>
                            <i className={`fas ${sectionsCollapsed.poolIO ? 'fa-chevron-down' : 'fa-chevron-up'}`}></i>
                        </span>
                    </button>
                </div>
            </div>
                    ...
                    <table className='table is-fullwidth is-striped'>
                        ...
                        <tbody>
                            {poolIOStats.map((poolIO, index) => {
                                ...
                                return (
                                    <tr key={index}>
                                        <td><strong>{poolIO.pool}</strong></td>
                                        <td>
                                            <span className={`tag ${poolIO.pool_type === 'raidz2' ? 'is-success' :
                                                    poolIO.pool_type === 'raidz1' ? 'is-info' :
                                                        poolIO.pool_type === 'mirror' ? 'is-warning' : 'is-dark'
                                                }`}>
                                                {poolIO.pool_type}
                                            </span>
                                        </td>
                                        ...
                                        <td>
                                            <span className='tag is-info'>
                                                {readBandwidthMB >= 1 ? `${readBandwidthMB.toFixed(2)} MB/s` :
                                                    readBandwidthMB > 0 ? `${(readBandwidthMB * 1024).toFixed(0)} KB/s` :
                                                        poolIO.read_bandwidth || '0 B/s'}
                                            </span>
                                        </td>
                                        <td>
                                            <span className='tag is-warning'>
                                                {writeBandwidthMB >= 1 ? `${writeBandwidthMB.toFixed(2)} MB/s` :
                                                    writeBandwidthMB > 0 ? `${(writeBandwidthMB * 1024).toFixed(0)} KB/s` :
