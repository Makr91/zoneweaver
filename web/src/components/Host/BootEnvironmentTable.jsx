...
const BootEnvironmentTable = ({ bootEnvironments, loading, onActivate, onMount, onUnmount, onDelete }) => {
  ...
  const getActiveIcon = (be) => {
    if (be.is_active_now && be.is_active_on_reboot) {
      return <span className='icon has-text-success'><i className='fas fa-check-double'></i></span>;
    }
    if (be.is_active_now) {
      return <span className='icon has-text-success'><i className='fas fa-check-circle'></i></span>;
    }
    if (be.is_active_on_reboot) {
      return <span className='icon has-text-info'><i className='fas fa-clock'></i></span>;
    }
  ...
  const getPolicyTag = (policy) => {
    switch (policy?.toLowerCase()) {
      ...
      default:
        return <span className='tag is-grey is-small'>{policy || 'Unknown'}</span>;
    }
  };
                ...
                <td>
                  <div className='is-flex is-align-items-center'>
                    ...
                    <span className='ml-2'>
                      ...
                      {be.is_temporary && (
                        <span className='tag is-warning is-small ml-2'>Temporary</span>
                      )}
                    </span>
                  </div>
                </td>
