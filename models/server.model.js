...
export default (sequelize, Sequelize) => {
  ...
  Server.addServer = async function({
    ...
    try {
      ...
      if (apiKey) {
        ...
        const testResult = await this.testServerWithApiKey(hostname, port, protocol, apiKey, allowInsecure);
        ...
        // Bootstrap API key
        const apiKeyResult = await this.bootstrapApiKey({ hostname, port, protocol, entityName, allowInsecure });
  ...
  Server.testServerWithApiKey = async function(hostname, port, protocol, apiKey, allowInsecure = false) {
    try {
  ...
  Server.bootstrapApiKey = async function({ hostname, port, protocol, entityName, allowInsecure = false }) {
    try {
      const serverUrl = `${protocol}://${hostname}:${port}`;
      ...
      if (error.response) {
        ...
        if (error.response.status === 403 && errorMessage.includes('Bootstrap endpoint auto-disabled')) {
          throw new Error('Bootstrap endpoint has been auto-disabled after first use for security. Please use the "I have an existing API key" option instead.');
        }
