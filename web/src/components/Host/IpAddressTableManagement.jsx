...
const IpAddressTable = ({ addresses, loading, onDelete, onToggle }) => {
  ...
  const getStateIcon = (state) => {
    switch (state?.toLowerCase()) {
      ...
      case 'duplicate':
        return <span className='icon has-text-danger'><i className='fas fa-exclamation-triangle'></i></span>;
      default:
  ...
  const getVersionTag = (version) => {
    switch (version?.toLowerCase()) {
      ...
      default:
        return <span className='tag is-grey is-small'>{version || 'Unknown'}</span>;
    }
  };

  const formatAddress = (addr, ipAddress, prefixLength) => {
    if (addr) return addr;
    if (ipAddress && prefixLength) return `${ipAddress}/${prefixLength}`;
    return ipAddress || 'N/A';
  };

  const canEnable = (address) => {
    return address.state?.toLowerCase() === 'disabled';
  };

  const canDisable = (address) => {
    return address.state?.toLowerCase() === 'ok';
  };
              ...
              <tr key={address.id || address.addrobj || index}>
                <td>
                  <div className='is-flex is-align-items-center'>
                    ...
                    <span className='ml-2'>
                      <strong className='is-family-monospace'>{address.interface}</strong>
                    </span>
                  </div>
                </td>
                <td>
                  <span className='is-family-monospace is-size-7'>{address.addrobj}</span>
                </td>
                <td>
                  <div>
                ...
                <td>
                  {getTypeTag(address.type)}
                </td>
                <td>
                  {getVersionTag(address.ip_version)}
                </td>
                <td>
                  {getStateTag(address.state)}
                </td>
                <td>
