      ...
      withStats: {
        ...
        attributes: {
          include: [
            ...
            [
              sequelize.fn('COUNT', sequelize.literal('CASE WHEN users.role = \'admin\' AND users.is_active = true THEN 1 END')),
              'admin_users'
            ]
          ]
        },
  ...
  Organization.prototype.getStats = async function() {
    const stats = await sequelize.models.user.findAll({
      ...
      attributes: [
        ...
        [sequelize.fn('COUNT', sequelize.literal('CASE WHEN role = \'admin\' AND is_active = true THEN 1 END')), 'admin_users']
      ],
      raw: true
    });
