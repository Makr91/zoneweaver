  ...
  const handleDeleteEtherstub = async (etherstubName) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        onError(result.message || `Failed to delete etherstub "${etherstubName}"`);
      }
    } catch (err) {
  ...
  return (
    <div>
      <div className='mb-4'>
        ...
        <p className='content'>
          Manage etherstubs on <strong>{server.hostname}</strong>. Etherstubs provide a virtual Layer 2 switch for connecting VNICs.
        </p>
      </div>
