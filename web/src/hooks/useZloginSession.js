...
export const useZloginSession = (
  ...
  const refreshZloginSessionStatus = useCallback(
    async (zoneName) => {
      ...
      try {
        console.log(
          `🔍 ZLOGIN STATUS: Checking session status for zone: ${zoneName}`
        );
        ...
        if (sessionsResult.success && sessionsResult.data) {
          ...
          console.log(
            `❌ ZLOGIN STATUS: No sessions found or API error for ${zoneName}`
          );
          setZoneDetails((prev) => {
            // 🛡️ DEFENSIVE STATE MERGE: Only clear zlogin fields, explicitly preserve VNC state
            const newState = {
        ...
        console.error(
          "💥 ZLOGIN STATUS: Error checking session status:",
          error
        );
        setZoneDetails((prev) => {
          // 🛡️ DEFENSIVE STATE MERGE: Only clear zlogin fields, explicitly preserve VNC state
