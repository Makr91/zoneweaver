...
const BridgeTable = ({ bridges, loading, onDelete, onViewDetails }) => {
  ...
  const getProtectionTag = (protection) => {
    switch (protection?.toLowerCase()) {
      ...
      default:
        return <span className='tag is-grey is-small'>{protection || 'Unknown'}</span>;
    }
  };

  const formatLinks = (links) => {
    if (!links || !Array.isArray(links)) return 'N/A';
    ...
    return (
      <div className='has-text-centered p-4'>
        <span className='icon is-large has-text-grey'>
          <i className='fas fa-bridge-water fa-2x'></i>
        </span>
          ...
          {bridges.map((bridge, index) => {
            ...
            return (
              <tr key={bridge.name || index}>
                ...
                <td>
                  {getProtectionTag(bridge.protection)}
                </td>
                <td>
                ...
                <td>
                  <span className='is-family-monospace is-size-7' title={bridge.links?.join(', ')}>
                    {formatLinks(bridge.links)}
                  </span>
                  {bridge.links && bridge.links.length > 0 && (
                    <div className='is-size-7 has-text-grey'>
                      {bridge.links.length} link{bridge.links.length !== 1 ? 's' : ''}
                    </div>
                  )}
                </td>
                <td>
                  <span className='is-size-7'>{bridge.max_age !== undefined ? `${bridge.max_age}s` : 'N/A'}</span>
                </td>
                <td>
                  <span className='is-size-7'>{bridge.hello_time !== undefined ? `${bridge.hello_time}s` : 'N/A'}</span>
                </td>
                <td>
                  <span className='is-size-7'>{bridge.forward_delay !== undefined ? `${bridge.forward_delay}s` : 'N/A'}</span>
                </td>
                <td>
