...
export const FooterProvider = ({ children }) => {
  ...
  const createSession = useCallback(async () => {
    ...
    if (
      ...
      console.log(
        "🚫 FOOTER: Session creation blocked - already exists or creating"
      );
      return;
    }
    ...
    try {
      ...
      ws.onopen = () => {
        console.log(
          "🔗 FOOTER: WebSocket connected for session:",
          sessionData.id
        );
        // Send initial prompt
        ws.send("\n");
  ...
  useEffect(() => {
    if (currentServer && !persistentSession.current) {
      console.log(
        "🛠️ FOOTER: Auto-creating session for server:",
        currentServer.hostname
      );
      createSession();
    }
