...
export const useHostNetworkingData = () => {
  ...
  const loadHistoricalChartData = useCallback(async () => {
    ...
    try {
      ...
      if (latestTimestamp) {
        ...
        console.log(
          "📊 HISTORICAL CHARTS: Set last timestamp to:",
          newTimestamp
        );
      }
    } catch (err) {
  ...
  const loadRecentNetworkData = useCallback(async () => {
    ...
    try {
      ...
      if (!lastChartTimestamp) {
        console.warn(
          "🔄 RECENT CHARTS: No last timestamp available, skipping recent data load."
        );
        setLoading(false);
        return;
      }
      ...
      if (result.success && result.data?.usage?.length > 0) {
        ...
        console.log(
          `🔄 RECENT CHARTS: Fetched ${recentUsage.length} new usage records.`
        );
      ...
      console.error(
        "💥 RECENT CHARTS: Error loading recent network data:",
        err
      );
    } finally {
      setLoading(false);
  ...
  useEffect(() => {
    ...
    if (autoRefresh && currentServer && initialDataLoaded) {
      ...
      interval = setInterval(() => {
        console.log(
          "🔄 NETWORKING: Auto-refreshing topology and recent chart data..."
        );
  ...
  useEffect(() => {
    if (currentServer && makeZoneweaverAPIRequest && initialLoadDone.current) {
      ...
      console.log(
        "📊 HISTORICAL CHARTS: Resetting last timestamp for full reload."
      );
      loadHistoricalChartData();
