...
const DeviceSummary = ({ deviceCategories, devicesSummary, pptStatus, sectionsCollapsed, toggleSection }) => {
                            ...
                            {Object.entries(deviceCategories).map(([category, stats]) => (
                                <div key={category} className='control'>
                                    <div className='tags has-addons'>
                                        <span className='tag'>{category.charAt(0).toUpperCase() + category.slice(1)}</span>
                                ...
                                <div className='tags has-addons'>
                                    ...
                                    <span className='tag is-success'>
                                        {devicesSummary.ppt_capable ||
                                            Object.values(deviceCategories).reduce((total, cat) => total + (cat.ppt_capable || 0), 0)}
                                    </span>
                                </div>
                            </div>
                            <div className='control'>
                                <div className='tags has-addons'>
                                    ...
                                    <span className='tag is-warning'>{pptStatus.summary?.available || 0}</span>
                                </div>
                            </div>
                            <div className='control'>
                                <div className='tags has-addons'>
                                    <span className='tag'>PPT Assigned</span>
                                    <span className='tag is-danger'>{devicesSummary.ppt_assigned || 0}</span>
                                </div>
                            </div>
                        </div>
                    </div>
