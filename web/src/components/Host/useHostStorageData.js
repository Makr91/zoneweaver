    ...
    try {
      ...
      if (poolsResult.status === "fulfilled" && poolsResult.value.success) {
        ...
        const deduplicatedPools = pools.reduce((acc, pool) => {
          const existing = acc.find(
            (existing) =>
              (existing.name || existing.pool) === (pool.name || pool.pool)
          );
          if (!existing) {
            acc.push(pool);
          } else {
      ...
      if (
        ...
        const deduplicatedDatasets = datasets.reduce((acc, dataset) => {
          const existing = acc.find(
            (existing) =>
              (existing.name || existing.dataset) ===
          ...
          if (!existing) {
            ...
            if (
              ...
              new Date(dataset.scan_timestamp) >
                new Date(existing.scan_timestamp)
            ) {
              const index = acc.indexOf(existing);
              acc[index] = dataset;
            }
      ...
      if (disksResult.status === "fulfilled" && disksResult.value.success) {
        ...
        const deduplicatedDisks = disks.reduce((acc, disk) => {
          ...
          const diskId =
            disk.device_name || disk.serial_number || disk.device || disk.name;
          const existing = acc.find(
            (existing) =>
              (existing.device_name === disk.device_name && disk.device_name) ||
              (existing.serial_number === disk.serial_number &&
                disk.serial_number) ||
        ...
        switch (chartSortBy) {
          ...
          case "bandwidth": {
            const aAvg =
              dataA.totalData
                .slice(-5)
                .reduce((sum, point) => sum + (point[1] || 0), 0) /
              Math.min(5, dataA.totalData.length);
            const bAvg =
              dataB.totalData
                .slice(-5)
                .reduce((sum, point) => sum + (point[1] || 0), 0) /
              Math.min(5, dataB.totalData.length);
          ...
          case "read": {
            const aAvg =
              dataA.readData
                .slice(-5)
                .reduce((sum, point) => sum + (point[1] || 0), 0) /
              Math.min(5, dataA.readData.length);
            const bAvg =
              dataB.readData
                .slice(-5)
                .reduce((sum, point) => sum + (point[1] || 0), 0) /
              Math.min(5, dataB.readData.length);
            return bAvg - aAvg; // Descending order (highest first)
          }
          ...
          case "write": {
            const aAvg =
              dataA.writeData
                .slice(-5)
                .reduce((sum, point) => sum + (point[1] || 0), 0) /
              Math.min(5, dataA.writeData.length);
            const bAvg =
              dataB.writeData
                .slice(-5)
                .reduce((sum, point) => sum + (point[1] || 0), 0) /
              Math.min(5, dataB.writeData.length);
            return bAvg - aAvg; // Descending order (highest first)
          }
      ...
      if (result.success && result.data?.diskio) {
        ...
        Object.entries(deviceGroups).forEach(([deviceName, records]) => {
          ...
          sortedRecords.forEach((io) => {
            ...
            newDiskChartData[deviceName].readData.push([
              timestamp,
              parseFloat(readMBps.toFixed(3)),
            ]);
            newDiskChartData[deviceName].writeData.push([
              timestamp,
              parseFloat(writeMBps.toFixed(3)),
            ]);
            newDiskChartData[deviceName].totalData.push([
              timestamp,
          ...
          console.log(
            `📊 DISK IO: Built ${sortedRecords.length} historical points for device ${deviceName}`
          );
        });
        ...
        const latestDiskIO = Object.entries(deviceGroups)
          .map(
            ([deviceName, records]) =>
              records.sort(
                (a, b) =>
        ...
        console.log(
          "📊 DISK IO: Loaded historical data for",
          Object.keys(newDiskChartData).length,
          "devices"
        );
      } else {
        setDiskIOStats([]);
  ...
  const loadPoolIOStats = async (server) => {
    ...
    try {
      ...
      if (historicalResult.success && historicalResult.data?.poolio) {
        ...
        console.log(
          "📊 POOL IO: Processing",
          historicalPoolIO.length,
          "historical pool IO records"
        );

        const validHistoricalPoolIO = historicalPoolIO.filter(
        ...
        Object.entries(poolData).forEach(([poolName, ioArray]) => {
          ...
          const sortedRecords = ioArray.sort(
            (a, b) => new Date(a.scan_timestamp) - new Date(b.scan_timestamp)
          );
          ...
          sortedRecords.forEach((io) => {
            ...
            historicalPoolChartData[poolName].readData.push([
              timestamp,
              parseFloat(readMBps.toFixed(3)),
            ]);
            historicalPoolChartData[poolName].writeData.push([
              timestamp,
              parseFloat(writeMBps.toFixed(3)),
            ]);
            historicalPoolChartData[poolName].totalData.push([
              timestamp,
              parseFloat(totalMBps.toFixed(3)),
            ]);
          });
          ...
          console.log(
            `📊 POOL IO: Built ${sortedRecords.length} historical points for pool ${poolName}`
          );
        });
        ...
        const latestPoolIOStats = Object.entries(poolData)
          .map(
            ([poolName, ioArray]) =>
              ioArray.sort(
                (a, b) =>
    ...
    diskIOData.forEach((io) => {
      ...
      if (chart && chart.chart) {
        ...
        try {
          if (chartObj.series[0]) {
            chartObj.series[0].addPoint(readPoint, false, shift);
          }
          if (chartObj.series[1]) {
            chartObj.series[1].addPoint(writePoint, false, shift);
          }
          if (chartObj.series[2]) {
            chartObj.series[2].addPoint(totalPoint, false, shift);
          }
          chartObj.redraw();
        } catch (error) {
          console.warn(
            `Error updating individual chart for ${deviceName}:`,
            error
          );
        }
      }
      ...
      ["read", "write", "total"].forEach((type, typeIndex) => {
        ...
        if (summaryChart && summaryChart.chart) {
          ...
          try {
            const deviceSeries = chart.series.find(
              (series) => series.name === deviceName
            );
            if (deviceSeries) {
            ...
            console.warn(
              `Error updating summary ${type} chart for ${deviceName}:`,
              error
            );
          }
        }
      ...
      setChartData((prevData) => {
        ...
        if (newData[deviceName].readData.length > maxDataPoints) {
          newData[deviceName].readData =
            newData[deviceName].readData.slice(-maxDataPoints);
          newData[deviceName].writeData =
            newData[deviceName].writeData.slice(-maxDataPoints);
          newData[deviceName].totalData =
            newData[deviceName].totalData.slice(-maxDataPoints);
        }
        return newData;
      });
    });
    ...
    poolIOData.forEach((poolIO) => {
      ...
      if (chart && chart.chart) {
        ...
        try {
          ...
          console.warn(
            `Error updating individual pool chart for ${poolName}:`,
            error
          );
        }
      }
  ...
  const updateArcChartData = (arcData) => {
    ...
    sortedArcData.forEach((arcRecord) => {
      ...
      historicalChartData.arcSize.push([
        timestamp,
        parseFloat(arcSizeGB.toFixed(2)),
      ]);
      historicalChartData.arcTargetSize.push([
        timestamp,
        parseFloat(arcTargetSizeGB.toFixed(2)),
      ]);
      historicalChartData.mruSize.push([
        timestamp,
        parseFloat(mruSizeGB.toFixed(2)),
      ]);
      historicalChartData.mfuSize.push([
        timestamp,
        parseFloat(mfuSizeGB.toFixed(2)),
      ]);
      historicalChartData.dataSize.push([
        timestamp,
        parseFloat(dataSizeGB.toFixed(2)),
      ]);
      historicalChartData.metadataSize.push([
        timestamp,
        parseFloat(metadataSizeGB.toFixed(2)),
      ]);
