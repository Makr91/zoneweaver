...
const NetworkTopologyViewer = ({ 
  ...
  const baseTopology = useMemo(() => {
    ...
    console.log('🔍 TOPOLOGY: Sample networkInterfaces data:', networkData.networkInterfaces?.slice(0, 3));
  ...
  const viewTopology = useMemo(() => {
    if (!baseTopology.nodes.length) return baseTopology;
  ...
  useEffect(() => {
    if (viewTopology.nodes.length > 0) {
      ...
      console.log('🔍 VIEWER: Input nodes:', viewTopology.nodes.map(n => ({ id: n.id, type: n.type, position: n.position })));
      ...
      console.log('🔍 VIEWER: Layout calculated, result:', layoutedTopology.nodes.map(n => ({ id: n.id, type: n.type, position: n.position })));
      ...
      const zoneEdges = layoutedTopology.edges.filter(e => e.target.includes('--') || e.source.includes('--'));
      console.log('🔍 VIEWER: Zone edges:', zoneEdges.map(e => ({ id: e.id, source: e.source, target: e.target, flowDirection: e.data?.flowDirection })));
        ...
        >
          ...
          {showMinimap && (
            <MiniMap 
              nodeColor={(node) => {
                switch (node.type) {
                  case 'physicalNic': return '#48c78e';
                  case 'aggregate': return '#3273dc';
                  case 'etherstub': return '#ffdd57';
                  case 'vnic': return '#ff9f43';
                  case 'zone': return '#f14668';
