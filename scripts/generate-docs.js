...
<head>
    ...
    <title>Zoneweaver Frontend API Reference</title>
    <link rel="icon" type="image/x-icon" href="/zoneweaver/assets/images/favicon.ico">
    <link rel="apple-touch-icon" sizes="192x192" href="/zoneweaver/assets/images/logo192.png">
    <link rel="apple-touch-icon" sizes="512x512" href="/zoneweaver/assets/images/logo512.png">
...
const generateDocs = () => {
  ...
  try {
    ...
    // Front matter emits the spec at the clean /api/openapi.json permalink.
    // layout:null = no theme wrapper; render_with_liquid:false = leave the JSON untouched.
    const openApiPage = `---
layout: null
...
---
${openApiJson}`;
    fs.writeFileSync(path.join(docsDir, 'openapi.json'), openApiPage);
...
// Run the documentation generation (synchronous; it calls process.exit(1) on failure)
generateDocs();
