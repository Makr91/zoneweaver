...
export const useVncSession = (
  ...
  const { makeZoneweaverAPIRequest, startVncSession, stopVncSession } =
    useServers();
  ...
  const refreshVncSessionStatus = useCallback(
    async (zoneName) => {
      ...
      try {
        console.log(
          `🔍 VNC STATUS: Checking session status for zone: ${zoneName}`
        );
      ...
      const pollValidation = async (attemptsLeft) => {
        ...
        try {
          ...
          if (vncResult.success && vncResult.data) {
            const hasActiveProperty =
              typeof vncResult.data.active !== "undefined";
  ...
  const handleVncClipboardPaste = (text) => {
    console.log(
      `VNC CLIPBOARD: Attempting to paste text of length ${text.length}`
    );
  };
