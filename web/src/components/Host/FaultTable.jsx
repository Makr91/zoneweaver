...
const FaultTable = ({ faults, loading, onAction, onViewDetails }) => {
  ...
  const getSeverityIcon = (severity) => {
    switch (severity.toLowerCase()) {
      case 'critical':
        return <span className='icon has-text-danger'><i className='fas fa-exclamation-circle'></i></span>;
      case 'major':
        return <span className='icon has-text-warning'><i className='fas fa-exclamation-triangle'></i></span>;
      case 'minor':
        return <span className='icon has-text-info'><i className='fas fa-info-circle'></i></span>;
      default:
  ...
  const getAvailableActions = () => {
    ...
    return [
      { key: 'acquit', label: 'Acquit', icon: 'fa-check', class: 'is-success', requiresConfirm: true },
      { key: 'repaired', label: 'Mark Repaired', icon: 'fa-wrench', class: 'is-info', requiresConfirm: true },
      { key: 'replaced', label: 'Mark Replaced', icon: 'fa-exchange-alt', class: 'is-warning', requiresConfirm: true }
    ];
  };

  const getFaultClass = (msgId) => {
    // Extract readable fault class from message ID
    if (msgId.startsWith('ZFS-')) return 'ZFS';
    if (msgId.startsWith('FMD-')) return 'FMD';
    if (msgId.startsWith('CPU-')) return 'CPU';
    if (msgId.startsWith('MEM-')) return 'Memory';
  ...
  if (faults.length === 0) {
    return (
      <div className='has-text-centered p-4'>
        ...
        <p className='mt-2 has-text-success'><strong>No system faults found</strong></p>
          ...
          {faults.map((fault, index) => {
            ...
            return (
              <tr key={fault.uuid || index}>
                ...
                <td>
                  <span className='is-family-monospace is-size-7' title={fault.uuid}>
