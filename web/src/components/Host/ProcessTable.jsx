...
const ProcessTable = ({ 
  ...
  const formatCPU = (cpuPercent) => {
    if (cpuPercent === null || cpuPercent === undefined) return 'N/A';
    return `${parseFloat(cpuPercent).toFixed(1)}%`;
  };

  const getUserIcon = (username) => {
    if (username === 'root') {
      return <span className='icon has-text-danger'><i className='fas fa-user-shield'></i></span>;
    }
    return <span className='icon has-text-info'><i className='fas fa-user'></i></span>;
  };
