...
const calculateHierarchicalLayout = (nodes, edges, viewType) => {
  ...
  console.log('🔍 LAYOUT: Nodes by type:', Object.fromEntries(
  ...
  const getJustifiedLayout = (nodeCount, defaultSpacing, rowType = 'default') => {
    ...
    if (maxRowWidth > normalRowWidth * 1.5) {
      ...
      console.log(`🔍 LAYOUT: Justifying ${rowType} row - nodes: ${nodeCount}, normal width: ${normalRowWidth}, justified width: ${justifiedRowWidth}, spacing: ${justifiedSpacing}`);
  ...
  if (nodesByType.physicalNic.length > 0) {
    const layout = getJustifiedLayout(nodesByType.physicalNic.length, horizontalSpacing, 'physicalNic');
    ...
    nodesByType.physicalNic.forEach((node, index) => {
      ...
      console.log(`🔍 LAYOUT: Placing physicalNic node ${node.id} at position`, position);
  ...
  if (nodesByType.aggregate.length > 0) {
    ...
    nodesByType.aggregate.forEach((node, index) => {
      ...
      console.log(`🔍 LAYOUT: Placing aggregate node ${node.id} at position`, position);
  ...
  if (nodesByType.etherstub.length > 0) {
    ...
    nodesByType.etherstub.forEach((node, index) => {
      ...
      console.log(`🔍 LAYOUT: Placing etherstub node ${node.id} at position`, position);
  ...
  if (nodesByType.vnic.length > 0) {
    ...
    nodesByType.vnic.forEach((node, index) => {
      ...
      console.log(`🔍 LAYOUT: Placing vnic node ${node.id} at position`, position);
  ...
  if (nodesByType.zone.length > 0) {
    ...
    nodesByType.zone.forEach((node, index) => {
      ...
      console.log(`🔍 LAYOUT: Placing zone node ${node.id} at position`, position);
  ...
  console.log('🔍 LAYOUT: Final vertical layout created with', layoutedNodes.length, 'positioned nodes');
...
const calculateZoneCentricLayout = (nodes, edges) => {
  ...
  const remainingNodes = nodes.filter(n => 
    !layoutedNodes.some(ln => ln.id === n.id)
  );
