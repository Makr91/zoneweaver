        ...
        <div className='box mb-4'>
            <div className='level is-mobile mb-3'>
                ...
                <div className='level-right'>
                    ...
                    >
                        <span className='icon'>
                            <i className={`fas ${sectionsCollapsed.diskIO ? 'fa-chevron-down' : 'fa-chevron-up'}`}></i>
                        </span>
                    </button>
                </div>
            </div>
                            ...
                            <table className='table is-fullwidth is-striped'>
                                ...
                                <tbody>
                                    {diskIOStats.map((io, index) => {
                                        ...
                                        return (
                                            <tr key={index}>
                                                <td><strong>{io.device_name}</strong></td>
                                                <td><span className='tag is-primary'>{io.pool}</span></td>
                                                <td>{io.read_ops}</td>
                                                <td>{io.write_ops}</td>
                                                <td>
                                                    <span className='tag is-info'>
                                                        {readMBps >= 1 ? `${readMBps.toFixed(2)} MB/s` :
                                                            readMBps > 0 ? `${(readMBps * 1024).toFixed(0)} KB/s` : '0 B/s'}
                                                    </span>
                                                </td>
                                                <td>
                                                    <span className='tag is-warning'>
                                                        {writeMBps >= 1 ? `${writeMBps.toFixed(2)} MB/s` :
                                                            writeMBps > 0 ? `${(writeMBps * 1024).toFixed(0)} KB/s` : '0 B/s'}
                                                    </span>
                                                </td>
                                                <td>
                                                    <span className={`tag ${totalMBps > 50 ? 'is-danger' :
                                                            totalMBps > 10 ? 'is-warning' :
                                                                totalMBps > 0 ? 'is-success' : 'is-grey'
                                                        }`}>
                                                        {totalMBps >= 1 ? `${totalMBps.toFixed(2)} MB/s` :
                                                            totalMBps > 0 ? `${(totalMBps * 1024).toFixed(0)} KB/s` : '0 B/s'}
                                                    </span>
                                                </td>
                                                <td>
