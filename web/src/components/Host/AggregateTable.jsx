              ...
              <tr key={aggregateName || index}>
                <td>
                  <div className='is-flex is-align-items-center'>
                    ...
                    <span className='ml-2'>
                      <strong className='is-family-monospace'>{aggregateName}</strong>
                    </span>
                  </div>
                </td>
                <td>
                  {getPolicyTag(aggregate.policy)}
                </td>
                <td>
                  <span className='is-family-monospace is-size-7' title={getLinksArray(memberLinks).join(', ')}>
                    {formatLinks(memberLinks)}
                  </span>
                  {getLinksArray(memberLinks).length > 0 && (
                    <div className='is-size-7 has-text-grey'>
                      {getLinksArray(memberLinks).length} link{getLinksArray(memberLinks).length !== 1 ? 's' : ''}
                    </div>
                  )}
                </td>
                <td>
                  {getStateTag(aggregate.state)}
                </td>
                <td>
                  {getLacpModeTag(aggregate.lacp_mode)}
                </td>
                <td>
                  <span className='is-size-7'>{aggregate.lacp_timer || 'N/A'}</span>
                </td>
                <td>
