        ...
        <div className='box mb-4'>
            <div className='level is-mobile mb-3'>
                <div className='level-left'>
                    ...
                    >
                        <span className='icon-text'>
                            <span className='icon'><i className='fas fa-hard-drive'></i></span>
                ...
                <div className='level-right'>
                    ...
                    >
                        <span className='icon'>
                            <i className={`fas ${sectionsCollapsed.disks ? 'fa-chevron-down' : 'fa-chevron-up'}`}></i>
                        </span>
                    </button>
                </div>
            </div>
                            ...
                            <table className='table is-fullwidth is-striped'>
                                <thead>
                                    <tr>
                                        ...
                                        >
                                            Pool <i className={`fas ${getSortIcon(diskSort, 'pool_assignment')}`}></i>
                                        </th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {storageDisks.map((disk, index) => (
                                        <tr key={index}>
                                            ...
                                            <td>
                                                <span className={`tag ${getHealthColor(disk.health || disk.status)}`}>
                                                    {disk.health || disk.status || 'Unknown'}
                                                </span>
                                            </td>
                                            <td>
                                                {disk.temperature ? (
                                                    <span className={`tag ${disk.temperature > 60 ? 'is-danger' :
                                                            disk.temperature > 45 ? 'is-warning' : 'is-success'
                                                        }`}>
                                                        {disk.temperature}°C
                                                    </span>
                                                ) : (
