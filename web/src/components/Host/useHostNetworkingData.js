...
export const useHostNetworkingData = () => {
  console.log(
    "🐛 DEBUG: useHostNetworkingData hook starting - Enhanced for topology"
  );
  ...
  useEffect(() => {
    if (!autoRefresh || !currentServer || !initialDataLoaded) {
      if (!initialDataLoaded) {
        console.log(
          "🔄 NETWORKING: Auto-refresh waiting for initial data load to complete"
        );
      }
      return;
    }
    ...
    console.log(
      "🔄 NETWORKING: Setting up auto-refresh every",
      refreshInterval,
      "seconds"
    );
  ...
  const loadNetworkData = async () => {
    ...
    try {
      ...
      console.log(
        "🔍 NETWORKING: Loading complete network data for",
      ...
      if (interfacesResult.status === "fulfilled") {
        console.log(
          "🔍 NETWORKING: Interfaces response value:",
        ...
        console.log(
          "🔍 NETWORKING: Sample interface structure:",
          interfaces[0]
        );

        const deduplicatedInterfaces = interfaces.reduce(
          (acc, netInterface) => {
            // Use link field as primary identifier since that's what the API provides
            const interfaceId = netInterface.link;
            const existing = acc.find(
              (existing) => existing.link === interfaceId
            );
            ...
            if (!existing) {
              ...
              if (
                ...
                console.log(
                  "🔍 NETWORKING: Updated interface with newer timestamp:",
                  interfaceId
                );
              }
            }
        ...
        console.log(
          "🔍 NETWORKING: After deduplication:",
          deduplicatedInterfaces.length,
          "unique interfaces"
        );
        console.log(
          "🔍 NETWORKING: Unique interface links:",
          deduplicatedInterfaces.map((i) => i.link)
        );
        setNetworkInterfaces(deduplicatedInterfaces);
      } else {
        console.error(
          "🔍 NETWORKING: Failed to load interfaces - promise rejected:",
          interfacesResult
        );
      }
      ...
      if (usageResult.status === "fulfilled") {
        ...
        console.log(
          "🔍 NETWORKING: Loaded",
          deduplicatedUsage.length,
          "unique usage entries from monitoring API"
        );
      } else {
        console.error(
          "🔍 NETWORKING: Failed to load usage - promise rejected:",
          usageResult
        );
      }
      ...
      if (ipResult.status === "fulfilled") {
        ...
        const deduplicatedIps = ips.reduce((acc, ip) => {
          ...
          const existing = acc.find(
            (existing) =>
              (existing.addrobj ||
                `${existing.interface}-${existing.ip_address}`) === ipId
          );
          if (!existing) {
            acc.push(ip);
          } else {
        ...
        console.log(
          "🔍 NETWORKING: Loaded",
          deduplicatedIps.length,
          "unique IP addresses from monitoring API"
        );
      } else {
      ...
      if (routesResult.status === "fulfilled") {
        ...
        const deduplicatedRoutes = routes.reduce((acc, route) => {
          ...
          const existing = acc.find(
            (existing) =>
              `${existing.destination}-${existing.gateway}-${existing.interface}` ===
              routeId
          );
          if (!existing) {
            acc.push(route);
        ...
        console.log(
          "🔍 NETWORKING: Loaded",
          deduplicatedRoutes.length,
          "unique routes from monitoring API"
        );
      } else {
      ...
      if (aggregatesResult.status === "fulfilled") {
        ...
        const deduplicatedAggregates = aggregates.reduce((acc, aggregate) => {
          ...
          const existing = acc.find(
            (existing) => (existing.name || existing.link) === aggregateId
          );
          if (!existing) {
            acc.push(aggregate);
          } else {
            ...
            if (
              ...
              existing.scan_timestamp &&
              new Date(aggregate.scan_timestamp) >
                new Date(existing.scan_timestamp)
            ) {
              const index = acc.indexOf(existing);
              acc[index] = aggregate;
      ...
      if (etherstubsResult.status === "fulfilled") {
        const etherstubs =
          etherstubsResult.value?.data?.etherstubs ||
          etherstubsResult.value?.etherstubs ||
          [];
        const deduplicatedEtherstubs = etherstubs.reduce((acc, etherstub) => {
          const etherstubId = etherstub.name || etherstub.link;
          const existing = acc.find(
            (existing) => (existing.name || existing.link) === etherstubId
          );
          if (!existing) {
            acc.push(etherstub);
          } else {
            ...
            if (
              ...
              existing.scan_timestamp &&
              new Date(etherstub.scan_timestamp) >
                new Date(existing.scan_timestamp)
            ) {
              const index = acc.indexOf(existing);
              acc[index] = etherstub;
      ...
      if (vnicsResult.status === "fulfilled") {
        const vnics =
          vnicsResult.value?.data?.vnics || vnicsResult.value?.vnics || [];
        const deduplicatedVnics = vnics.reduce((acc, vnic) => {
          const vnicId = vnic.name || vnic.link;
          const existing = acc.find(
            (existing) => (existing.name || existing.link) === vnicId
          );
          if (!existing) {
            acc.push(vnic);
          } else {
      ...
      if (zonesResult.status === "fulfilled") {
        const zones =
          zonesResult.value?.data?.zones || zonesResult.value?.zones || [];
        const deduplicatedZones = zones.reduce((acc, zone) => {
          const zoneId = zone.name || zone.zonename;
          const existing = acc.find(
            (existing) => (existing.name || existing.zonename) === zoneId
          );
          if (!existing) {
            acc.push(zone);
          } else {
  ...
  const loadHistoricalChartData = async () => {
    ...
    try {
      ...
      console.error(
        "📊 HISTORICAL CHARTS: Error loading historical data:",
        error
      );
    }
  };
    ...
    setChartData((prevChartData) => {
      ...
      networkUsage.forEach((usage) => {
        ...
        if (interfaceName) {
          ...
          console.log(
            `📊 CHARTS: Adding real-time point for ${interfaceName} - RX: ${rxMbps}Mbps, TX: ${txMbps}Mbps`
          );
          ...
          console.log(
            `📊 CHARTS: Updated ${interfaceName}: RX=${rxMbps.toFixed(3)}Mbps, TX=${txMbps.toFixed(3)}Mbps (${interfaceData.rxData.length} points)`
          );
        }
      });

      console.log(
        "📊 CHARTS: Updated real-time chart data for",
        Object.keys(newChartData).length,
        "interfaces"
      );
      return newChartData;
  ...
  useEffect(() => {
    if (currentServer && makeZoneweaverAPIRequest && initialLoadDone.current) {
      ...
      console.log(
        "📊 HISTORICAL CHARTS: Skipping settings change during initial load"
      );
    }
