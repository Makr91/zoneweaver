...
const PackageTable = ({ packages, loading, onInstall, onUninstall, onViewDetails, isSearchMode }) => {
  ...
  const getStatusIcon = (pkg) => {
    if (pkg.installed && pkg.manually_installed) {
      return <span className='icon has-text-warning'><i className='fas fa-star'></i></span>;
    }
    if (pkg.installed) {
      return <span className='icon has-text-success'><i className='fas fa-check-circle'></i></span>;
    }
    if (pkg.frozen) {
      return <span className='icon has-text-info'><i className='fas fa-snowflake'></i></span>;
    }
  ...
  const getAvailableActions = (pkg) => {
    ...
    if (pkg.installed) {
      ...
      if (!pkg.frozen) {
        actions.push({ key: 'uninstall', label: 'Uninstall', icon: 'fa-trash', class: 'has-background-danger-dark has-text-danger-light' });
      }
    } else {
      // Package is not installed - can install
      actions.push({ key: 'install', label: 'Install', icon: 'fa-download', class: 'has-background-success-dark has-text-success-light' });
    }
              ...
              <tr key={pkg.name || index}>
                <td>
                  <div className='is-flex is-align-items-center'>
                    {getStatusIcon(pkg)}
                    <span className='ml-2'>
                      <strong className='is-family-monospace'>{pkg.name}</strong>
                    </span>
                  </div>
                </td>
                <td>
                ...
                <td>
                  <span className='is-family-monospace is-size-7'>{pkg.version || 'N/A'}</span>
                </td>
                <td>
                  {getStatusTag(pkg)}
                </td>
