...
const ProcessDetailsModal = ({ process, server, onClose }) => {
  ...
  const loadAdditionalData = async (type) => {
    if (!server || !makeZoneweaverAPIRequest || additionalData[type] !== null) return;
  ...
  const formatMemory = (bytes) => {
    ...
    if (gb >= 1) return `${gb.toFixed(2)} GB (${bytes} bytes)`;
    if (mb >= 1) return `${mb.toFixed(2)} MB (${bytes} bytes)`;
    if (kb >= 1) return `${kb.toFixed(2)} KB (${bytes} bytes)`;
    return `${bytes} bytes`;
  };
            ...
            <tbody>
              {additionalData.files.map((file, index) => (
                <tr key={index}>
                  ...
                  <td className='is-family-monospace is-size-7'>{file.description}</td>
                  <td className='is-family-monospace is-size-7'>{file.details}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <div className='box'>
        ...
        <div className='table-container'>
          <table className='table is-fullwidth'>
            ...
            <tbody>
              {Object.entries(additionalData.limits).map(([resource, limit], index) => (
                <tr key={index}>
  ...
  const renderStackTab = () => {
    ...
    return (
      <div className='box'>
        ...
        <pre className='is-size-7 p-3 has-background-light' style={{ maxHeight: '400px', overflow: 'auto' }}>
          {typeof additionalData.stack === 'string' ? additionalData.stack : JSON.stringify(additionalData.stack, null, 2)}
        </pre>
      </div>
    );
  };
