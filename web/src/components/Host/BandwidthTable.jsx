        ...
        <div className='box mb-4'>
            <div className='level is-mobile mb-3'>
                ...
                <div className='level-right'>
                    ...
                    >
                        <span className='icon'>
                            <i className={`fas ${sectionsCollapsed.bandwidth ? 'fa-chevron-down' : 'fa-chevron-up'}`}></i>
                        </span>
                    </button>
                </div>
            </div>
            {!sectionsCollapsed.bandwidth && (
                networkUsage.length > 0 ? (
                        ...
                        <table className='table is-fullwidth is-striped'>
                            <thead>
                                <tr>
                                    ...
                                    >
                                        TX Packets <i className={`fas ${getSortIcon(bandwidthSort, 'opackets_delta')}`}></i>
                                    </th>
                                </tr>
                            </thead>
                            <tbody>
                                {networkUsage.map((usage, index) => {
                                    ...
                                    return (
                                        <tr key={index}>
                                            <td><strong>{usage.link}</strong></td>
                                            <td>
                                                <span className={`tag ${getBandwidthColor(bandwidth.totalMbps)}`}>
                                            ...
                                            <td>{usage.time_delta_seconds ? `${usage.time_delta_seconds.toFixed(1)}s` : 'N/A'}</td>
                                            <td>
                                                <span className='has-text-info'>
                                                    {usage.ipackets_delta ? parseInt(usage.ipackets_delta).toLocaleString() : '0'}
                                                </span>
                                            </td>
                                            <td>
                                                <span className='has-text-warning'>
                                                    {usage.opackets_delta ? parseInt(usage.opackets_delta).toLocaleString() : '0'}
                                                </span>
                                            </td>
                                        </tr>
                                    );
                    ...
                    <div className='notification is-info'>
                        <p>No real-time bandwidth data available. The backend may still be collecting delta measurements.</p>
                    </div>
                )
            )}
        </div>
