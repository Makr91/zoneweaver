...
const ConfirmActionModal = ({ bootEnvironment, action, onClose, onConfirm }) => {
    ...
    switch (action) {
      ...
      case 'mount':
        return {
          ...
          warning: 'This will make the boot environment filesystem accessible for inspection or modification.'
        };
      case 'unmount':
        return {
  ...
  return (
    <FormModal
      ...
      submitVariant={actionDetails.buttonClass.includes('danger') ? 'is-danger' : actionDetails.buttonClass.includes('warning') ? 'is-warning' : actionDetails.buttonClass.includes('success') ? 'is-success' : 'is-info'}
      loading={loading}
    >
              ...
              <table className='table is-fullwidth'>
                <tbody>
                  ...
                  <tr>
                    ...
                    <td>
                      {bootEnvironment.is_active_now && bootEnvironment.is_active_on_reboot && (
                        <span className='tag is-success is-small'>Active Now + Reboot</span>
                      )}
                      {bootEnvironment.is_active_now && !bootEnvironment.is_active_on_reboot && (
                        <span className='tag is-success is-small'>Active Now</span>
                      )}
                      {!bootEnvironment.is_active_now && bootEnvironment.is_active_on_reboot && (
                        <span className='tag is-info is-small'>Active on Reboot</span>
                      )}
                      {!bootEnvironment.is_active_now && !bootEnvironment.is_active_on_reboot && (
                  ...
                  <tr>
                    ...
                    <td className='is-family-monospace'>{bootEnvironment.mountpoint === '-' ? 'Not Mounted' : bootEnvironment.mountpoint}</td>
                  </tr>
                  <tr>
          ...
          {/* Action Description */}
          <div className={`notification ${action === 'delete' ? 'is-danger' : 'is-info'}`}>
          ...
          {action === 'activate' && (
            <div className='box'>
              ...
              <div className='field'>
                ...
                <p className='help'>If checked, the system will revert to the previous BE after the next reboot.</p>
              </div>
            </div>
          )}
          ...
          {action === 'mount' && (
            <div className='box'>
              ...
              <div className='field'>
                ...
                <p className='help'>Directory where the boot environment will be mounted</p>
              </div>
          ...
          {action === 'unmount' && (
            <div className='box'>
              ...
              <div className='field'>
                ...
                <p className='help'>Use this if the normal unmount fails due to busy files.</p>
              </div>
            </div>
          )}
